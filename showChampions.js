    ...
    try{
        const champions = await fetch(url).then(resp=>resp.json()).then(data=>{
            data.forEach(element => {
                const {name, tileIcon, traits, apiName, cost, ability, squareIcon} = element;
                if(cost >= 1 && cost <= 5){
                    
                    const imagenElem = document.createElement("img");
                    const src = urlDdragon + (squareIcon.replace("Splashes/","Splashes/patching/").replace("_Mobile.","_mobile_small.") || tileIcon).toLowerCase().replace(".dds",".png").replace("tex","png");
                    imagenElem.src=src;
